...
// actually has a chronology. The global timeline ships in every gallery
// package whatever its flags say, so the check is a lookup rather than a
// request — a Set of `country_id`, so the countries served by both
// chronologies count once.
      ...
      <div class="timeline-link-box" v-if="showTimelineLink(filters)">
        ...
        <p>
          ...
          <RouterLink :to="{ name: 'timeline-results', query: { country: filters.country, begin: filters.from, end: filters.to } }">
            {{ $t('gallery.section.timeline') }} | {{ labelOf('countries', countryIdForCode(filters.country)) }}
          </RouterLink>
        </p>
      </div>
    </template>
