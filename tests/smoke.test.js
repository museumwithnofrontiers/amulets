...
describe('website smoke test', () => {
  ...
  it('renders a keyword search on the composed results view', async () => {
    ...
    const term = items[0].internal_name.split(' ')[0]
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain('Amulets and Talismans')
  ...
  it('renders the item sheet on the composed record view', async () => {
    ...
    // inventory-app#1827: the outside-reference chip is named from the
    // manifest by `project_id` (composables/gallery.js's
    // `itemSheet.related.outsideChip`, viewer-layout's
    // `useProjects().label(ref.project_id)`) — the legacy
    // `backward_compatibility` code is no longer rendered.
    ...
    const outsideChipText = outsideRow.textContent.replace(sharedTexts.en['gallery.results.notInThisGallery'], '').trim()
    expect(outsideChipText).toBe(manifest.projects[outsideReference.project_id].name.en)
    expect(outsideRow.querySelector('code')).toBeNull()
    app.unmount()
  }, 60000)
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    // An amulet with the Ottoman dynasty attached — a record whose
    // dynasty translation actually carries a history, so it survives
    // ItemSheet.vue's own "has something to show" filter.
    const { app, host } = await mountSite('#/item/eeb73625-9b7f-5b6e-bac6-70835ba47781')
  ...
  it('renders the timeline results on the composed timeline view', async () => {
    ...
    // The join to member items finds Greece's two dated objects even with
  ...
  it('renders the timeline gallery on the composed results view', async () => {
    ...
    expect(host.textContent).toContain('Amulet')
    app.unmount()
  }, 60000)
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    // page on this site actually reads is listed, not just amulets' own — a
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['amulets', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
